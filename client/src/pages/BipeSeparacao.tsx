...
export default function BipeSeparacao() {
  ...
  const [divHomologada, setDivHomologada] = useState(true);
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <Label className="text-xs">Necessidade do cliente</Label>
                <select
                  className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm"
                  value={divNecessidadeCli}
                  onChange={(e) => setDivNecessidadeCli(e.target.value)}
                >
                  <option>Nenhuma</option>
                  <option>Informar</option>
                  <option>Aprovação obrigatória</option>
                </select>
              </div>
            </div>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={divHomologada}
                onChange={(e) => setDivHomologada(e.target.checked)}
              />
              Substituição homologada
            </label>
            <div>
