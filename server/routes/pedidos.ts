  ...
  try {
    ...
    const itens = await db.prepare(`
      SELECT
        ...
        (CASE
          ...
          -- Corte tem estados próprios (não cai nos estados genéricos de troca)
          WHEN EXISTS(SELECT 1 FROM AD_TROCAITEM T WHERE T.NUNOTA=I.NUNOTA AND T.SEQUENCIA=I.SEQUENCIA AND T.TIPODIVERG='Corte' AND T.STATUS='APROVADO') THEN 'CORTE_APROVADO'
          WHEN EXISTS(SELECT 1 FROM AD_TROCAITEM T WHERE T.NUNOTA=I.NUNOTA AND T.SEQUENCIA=I.SEQUENCIA AND T.TIPODIVERG='Corte' AND T.STATUS IN ('PENDENTE','BLOQUEADO')) THEN 'AGUARDANDO_CORTE_COMERCIAL'
          WHEN EXISTS(SELECT 1 FROM AD_TROCAITEM T WHERE T.NUNOTA=I.NUNOTA AND T.SEQUENCIA=I.SEQUENCIA AND T.STATUS='BLOQUEADO' AND COALESCE(T.TIPODIVERG,'') <> 'Corte') THEN 'DIVERGENCIA_ENCAMINHADA'
          WHEN EXISTS(SELECT 1 FROM AD_TROCAITEM T WHERE T.NUNOTA=I.NUNOTA AND T.SEQUENCIA=I.SEQUENCIA AND T.STATUS='PENDENTE' AND COALESCE(T.TIPODIVERG,'') <> 'Corte') THEN 'EM_TRATATIVA_DIVERGENCIA'
          WHEN EXISTS(SELECT 1 FROM AD_TROCAITEM T WHERE T.NUNOTA=I.NUNOTA AND T.SEQUENCIA=I.SEQUENCIA AND T.STATUS='APROVADO' AND COALESCE(T.TIPODIVERG,'') <> 'Corte') THEN 'TROCA_APROVADA'
          WHEN EXISTS(SELECT 1 FROM AD_FALTAITEM F WHERE F.NUNOTA=I.NUNOTA AND F.SEQUENCIA=I.SEQUENCIA AND F.STATUS='RESOLVIDO' AND F.ACAO='APANHO') THEN 'APANHO_CONFERIDO'
          ...
          WHEN EXISTS(SELECT 1 FROM AD_TROCAITEM T WHERE T.NUNOTA=I.NUNOTA AND T.SEQUENCIA=I.SEQUENCIA AND T.STATUS='REJEITADO' AND COALESCE(T.TIPODIVERG,'') <> 'Corte') THEN 'TROCA_REJEITADA'
    ...
    for (const it of itens) {
      const fd = await db.prepare(
        "SELECT NUFLUXODIST, CODPRODNF, CODPRODFISICO, EANFISICO FROM AD_FLUXODISTINTO WHERE NUNOTA=? AND SEQUENCIA=? AND STATUS='APROVADO' ORDER BY NUFLUXODIST DESC LIMIT 1",
      ...
      if (fd) {
        ...
        // Marca física real: vem do código de barras bipado (TGFBAR.MARCA); fallback = marca do produto.
        const barFisico = fd.EANFISICO
          ? await db.prepare("SELECT MARCA FROM TGFBAR WHERE CODBARRAS=?").get(fd.EANFISICO) as any
          : null;
        ...
        it.fluxoDistinto = {
          ...
          codProdFisico: fd.CODPRODFISICO, descFisico: pf?.DESCRPROD,
          eanFisico: fd.EANFISICO ?? pf?.REFERENCIA, marcaFisico: barFisico?.MARCA ?? pf?.MARCA,
