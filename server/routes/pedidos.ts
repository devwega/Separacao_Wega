 ...
 */
router.get("/embarcacoes", (_req, res) => {
  try {
    const db = getDb();
 ...
 */
router.post("/:nunota/iniciar-separacao", (req, res) => {
  try {
    const db = getDb();
    const nunota = Number(req.params.nunota);
    const { codusu = 1 } = req.body ?? {};

    const cab = db.prepare("SELECT AD_STATUSSEP FROM TGFCAB WHERE NUNOTA=?").get(nunota) as any;
    if (!cab) {
      res.status(404).json({ error: "Pedido não encontrado" });
      return;
    }
 ...
 */
router.post("/:nunota/devolver-ajuste", (req, res) => {
  try {
    const db = getDb();
    const nunota = Number(req.params.nunota);
 ...
 */
router.get("/:nunota/auditoria", (req, res) => {
  try {
    const db = getDb();
    const nunota = Number(req.params.nunota);
    ...
    const cab = db.prepare("SELECT DTFATUR FROM TGFCAB WHERE NUNOTA=?").get(nunota) as any;
