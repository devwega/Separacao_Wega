...
router.get("/:nunota", (req, res) => {
  ...
  // Itens conformes
  const itensConformes = db.prepare(`
    SELECT
  ...
  // Itens substituídos
  const itensSubstituidos = db.prepare(`
    SELECT
  ...
  // Fluxo distinto
  const itensFluxoDistinto = db.prepare(`
    SELECT
 ...
 */
router.post("/:nunota/liberar", (req, res) => {
  const db = getDb();
  const nunota = Number(req.params.nunota);
