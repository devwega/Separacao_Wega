 ...
 */
router.post("/:id/informar-cliente", (req, res) => {
  const db = getDb();
  const id = Number(req.params.id);
  const r = db.prepare(`
    UPDATE AD_TROCAITEM
 ...
 */
router.post("/:id/registrar-aprov-cliente", (req, res) => {
  const db = getDb();
  const id = Number(req.params.id);
  const r = db.prepare(`
    UPDATE AD_TROCAITEM
 ...
 */
router.post("/:id/encaminhar-gestor", (req, res) => {
  const db = getDb();
  const id = Number(req.params.id);
  ...
  db.prepare(`UPDATE AD_TROCAITEM SET STATUS='BLOQUEADO' WHERE NUTROCAITEM = ?`).run(id);
