 ...
 */
router.post("/validar-ean", (req, res) => {
  try {
    const db = getDb();
    ...
    if (item.eanEsperado === ean) {
      ...
      if (bar && bar.CODPROD === item.CODPROD) {
        ...
        outroProduto = db.prepare("SELECT CODPROD, DESCRPROD FROM TGFPRO WHERE REFERENCIA = ?").get(ean);
      }
    }
 ...
 */
router.post("/registrar-divergencia", (req, res) => {
  try {
    const db = getDb();
    ...
    const prodSubst = db.prepare("SELECT CODPROD FROM TGFPRO WHERE CODPROD=?").get(Number(b.codProdSubst));
    if (!prodSubst) {
 ...
 */
router.post("/registrar-falta", (req, res) => {
  try {
    const db = getDb();
    const b = req.body as any;
    const item = db.prepare(
    ...
    const ord = db.prepare("SELECT DTPREVSAIDA FROM TGFORD WHERE ORDEMCARGA=?").get(item.ORDEMCARGA) as any;
    const r = db.prepare(`
...
router.put("/conferir", (req, res) => {
  try {
    ...
    db.prepare("UPDATE TGFCAB SET AD_PERCPROGRESSO=?, AD_STATUSSEP=? WHERE NUNOTA=?")
 ...
 */
router.get("/saldo/:codprod", (req, res) => {
  try {
    const db = getDb();
